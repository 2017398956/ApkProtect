    ...
    private boolean replaceDexFiles(String zipPath,String newDexPath, String shellDexFile){
        ...
        try {
            ...
            if (!boolArm[0] && !boolArm[1] && !boolArm[2] && !boolArm[3]){
                Zip4jUtil.addFile2Zip(zipPath,OUT_TMP+"shell/lib/armeabi-v7a/libsxjiagu.so","lib/armeabi-v7a/");
            }else {
                if (boolArm[0]){
                    Zip4jUtil.addFile2Zip(zipPath,OUT_TMP+"shell/lib/armeabi-v7a/libsxjiagu.so","lib/armeabi-v7a/");
                }
                if (boolArm[1]){
                    Zip4jUtil.addFile2Zip(zipPath,OUT_TMP+"shell/lib/arm64-v8a/libsxjiagu.so","lib/arm64-v8a/");
                }
                if (boolArm[2]){
                    Zip4jUtil.addFile2Zip(zipPath,OUT_TMP+"shell/lib/x86/libsxjiagu.so","lib/x86/");
                }
                if (boolArm[3]){
                    Zip4jUtil.addFile2Zip(zipPath,OUT_TMP+"shell/lib/x86_64/libsxjiagu.so","lib/x86_64/");
                }
            }
